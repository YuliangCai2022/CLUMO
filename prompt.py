...
class Prompt(nn.Module):
    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None, word_idx = None):
        out = dict()
        if self.prompt_pool:
            ...
            if prompt_mask is None:
                ...
                if False:#self.batchwise_prompt:
            ...
            if word_idx is not None:
                idx = torch.reshape(torch.tensor(word_idx,dtype=torch.int),(-1,1)).to("cuda")
            ...
            for norm in out['selected_key']:
                cos_sim += cos(x_embed_norm,norm).sum()
            out['cos_sim'] = cos_sim
...
class DualKeyPrompt(nn.Module):
    def __init__(self, length=5, embed_dim=768, embedding_key='cls', prompt_init='uniform', prompt_pool=False, 
        ...
        if prompt_key:
            ...
            if prompt_key_init == 'zero':
                ...
                self.img_prompt_key = nn.Parameter(torch.zeros(key_shape))
            elif prompt_key_init == 'uniform':
                self.text_prompt_key = nn.Parameter(torch.randn(key_shape))
                self.img_prompt_key = nn.Parameter(torch.randn(key_shape))
    ...
    def forward(self, text_embed, img_embed, prompt_mask=None, cls_features=None, word_idx = None, add_prompt = False):
        ...
        if self.prompt_pool:
            ...
            # Put pull_constraint loss calculation inside
            batched_text_key_norm = text_prompt_norm[idx] # B, top_k, C
            out['selected_text_key'] = batched_text_key_norm
            batched_img_key_norm = img_prompt_norm[idx] # B, top_k, C
...
class DualKeyPrompt_cluster(nn.Module):
    def __init__(self, length=5, embed_dim=768, embedding_key='cls', prompt_init='uniform', prompt_pool=False, 
                 prompt_key=False, image_key_size=False, text_key_size=False, top_k=None, batchwise_prompt=False, prompt_key_init='uniform',):
        super().__init__()
